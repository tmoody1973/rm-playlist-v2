...
| -------------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Spinitron API keys (per station)             | Convex env vars                                          | `HYFIN_SPINITRON_KEY`, `STATION_88NINE_SPINITRON_KEY`, `STATION_414MUSIC_SPINITRON_KEY`, `RHYTHMLAB_SPINITRON_KEY`     | One per station; rotate per station independently. `STATION_` prefix on 88Nine/414Music because leading-digit env var names violate POSIX shell identifier rules (bash can't `export 88NINE_...`).                                                                                                                                                                                                                                                                                                                                                                     |
| SGmetadata (StreamGuys paid metadata API)    | Convex env var (key only) + DB per-stream scraper UUID   | `SGMETADATA_API_KEY`                                                                                                   | Per SGmetadata API PDF (rev 1.0.1, pages 2 + 13): one account-level API key, one scraper UUID per station. **The API key lives in the URL path** at `https://jetapi.streamguys.com/<API_KEY>/scraper/<SCRAPER_UUID>/metadata` — effectively a bearer token, must be treated as a secret. Base URL (`jetapi.streamguys.com`) is universal — do NOT override unless StreamGuys tech support specifically instructs. Per-stream scraper UUIDs (HYFIN, 88Nine, Rhythm Lab, 414 Music) are identifiers, not secrets — live in `ingestionSources.config.scraperUuid` per row |
| ICY stream URL                               | Convex env vars                                          | `RHYTHMLAB_ICY_URL`                                                                                                    | Not technically a secret but treated as one (private endpoint)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Ticketmaster Discovery API                   | Convex env vars                                          | `TICKETMASTER_CONSUMER_KEY`, `TICKETMASTER_CONSUMER_SECRET`                                                            | Free-tier rate-limited; rotate annually. OAuth-style consumer pair — key goes in `apikey` query param for Discovery v2; secret unused on public endpoints but stored for future auth'd endpoints.                                                                                                                                                                                                                                                                                                                                                                      |
